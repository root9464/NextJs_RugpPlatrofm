              ...
              {formatter && item?.value !== undefined && item.name ? (
                ...
                <>
                  ...
                  <div className={cn('flex flex-1 justify-between leading-none', nestLabel ? 'items-end' : 'items-center')}>
                    ...
                    {item.value && <span className='text-fg font-mono font-medium tabular-nums'>{item.value.toLocaleString()}</span>}
                  </div>
                </>
              )}
            </div>
  ...
  return (
    <div ref={ref} className={cn('flex items-center justify-center gap-4', verticalAlign === 'top' ? 'pb-3' : 'pt-3', className)}>
      {payload.map((item) => {
        ...
        return (
          <div key={item.value} className={cn('*:data-[slot=icon]:text-muted-fg flex items-center gap-1.5 *:data-[slot=icon]:size-3')}>
...
export { Chart, ChartLegend, ChartLegendContent, ChartStyle, ChartTooltip, ChartTooltipContent };
