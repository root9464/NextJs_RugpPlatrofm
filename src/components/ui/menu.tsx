...
const MenuContent = <T extends object>({ className, showArrow = false, popoverClassName, ...props }: MenuContentProps<T>) => {
          ...
          <>
            {values.selectionMode === 'single' && (
              <span
                ...
                {values.isSelected && <IconBulletFill data-slot='indicator' />}
              </span>
            )}
            {values.selectionMode === 'multiple' && (
              <div className='-mx-0.5 mr-2 flex size-4 shrink-0 items-center justify-center' data-slot='checkbox-icon'>
                {values.isSelected ? <IconCheck data-slot='checked-icon' /> : null}
              </div>
            )}
          </>
