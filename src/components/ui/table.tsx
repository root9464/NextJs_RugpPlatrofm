...
const TableHeader = <T extends object>({ children, ref, className, columns, ...props }: TableHeaderProps<T>) => {
  ...
  return (
    <TableHeaderPrimitive data-slot='table-header' ref={ref} className={header({ className })} {...props}>
      ...
      {selectionBehavior === 'toggle' && <Column className='w-0 pl-4'>{selectionMode === 'multiple' && <Checkbox slot='selection' />}</Column>}
...
const TableRow = <T extends object>({ children, className, columns, id, ref, ...props }: TableRowProps<T>) => {
  ...
  return (
    <Row
      ...
      {...props}
      className={row({
        className: 'href' in props ? cn('data-hovered:bg-secondary/50 data-hovered:text-secondary-fg cursor-pointer', className) : '',
      })}>
      {allowsDragging && (
        <Cell className='group ring-primary cursor-grab pr-0 data-dragging:cursor-grabbing'>
          <Button className='text-muted-fg data-pressed:text-fg relative bg-transparent py-1.5 pl-3.5' slot='drag'>
            <IconHamburger />
          </Button>
        </Cell>
      )}
      {selectionBehavior === 'toggle' && (
        <Cell className='pl-4'>
          <span aria-hidden className='bg-primary absolute inset-y-0 left-0 hidden h-full w-0.5 group-data-selected:block' />
