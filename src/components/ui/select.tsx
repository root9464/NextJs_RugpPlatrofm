...
import type { ListBoxProps, PopoverProps, SelectProps as SelectPrimitiveProps, ValidationResult } from 'react-aria-components';
...
const Select = <T extends object>({ label, description, errorMessage, className, ...props }: SelectProps<T>) => {
  return (
    <SelectPrimitive {...props} className={composeTailwindRenderProps(className, 'group flex w-full flex-col gap-y-1.5')}>
      {(values) => (
        <>
...
const SelectList = <T extends object>({ children, items, className, popoverClassName, ...props }: SelectListProps<T>) => {
  return (
    <PopoverContent
      showArrow={false}
      respectScreen={false}
      className={cn('border-uiMutedPrimary border sm:min-w-(--trigger-width)', popoverClassName)}
  ...
  return (
    <Button
      ...
      <SelectValue
        ...
        className='data-placeholder:text-uiSecondaryText grid flex-1 grid-cols-[auto_1fr] items-center text-base *:data-[slot=avatar]:*:-mx-0.5 *:data-[slot=avatar]:-mx-0.5 *:data-[slot=avatar]:*:mr-2 *:data-[slot=avatar]:mr-2 *:data-[slot=icon]:-mx-0.5 *:data-[slot=icon]:mr-2 sm:text-sm [&_[slot=description]]:hidden'
      />
      <IconChevronLgDown
        aria-hidden
        className='text-uiSecondaryText group-data-open:text-fg size-4 shrink-0 duration-300 group-data-disabled:opacity-50 group-data-open:rotate-180 forced-colors:text-[ButtonText] forced-colors:group-data-disabled:text-[GrayText]'
      />
    </Button>
  );
};
